...
const iconButtonBase =
  'inline-flex h-10 w-10 items-center justify-center rounded-full border border-[var(--sv-hairline)] text-[var(--sv-muted)] transition-colors duration-200 hover:border-[var(--sv-accent-line)] hover:text-[var(--sv-accent)]';
...
export function PortCard({ port, index, links }: PortCardProps) {
  return (
    <Card
      className="grid animate-[fadeUp_0.6s_ease_forwards] gap-4 p-6 opacity-0 transition-colors duration-200 hover:border-[var(--sv-hairline-strong)]"
      style={{ animationDelay: `${index * 0.05}s` }}
    >
      <div className="flex items-baseline justify-between gap-2.5">
        <h3 className="text-[1.5rem] text-[var(--sv-bright-white)]">
          {port.displayName}
        </h3>
        {links.releaseUrl ? (
          <a
            className="mono inline-flex items-center gap-1 text-[0.8rem] text-[var(--sv-muted)] transition-colors duration-200 hover:text-[var(--sv-accent)]"
          ...
          <span className="mono text-[0.8rem] text-[var(--sv-muted)]">
            v{port.version}
          </span>
        )}
      </div>
      <p className="m-0 leading-[1.55] text-[var(--sv-muted)]">
        {port.description}
      </p>
      ...
      <div className="mt-1 flex items-center gap-2">
        <a
          ...
          href={links.codeUrl}
          aria-label={`View ${port.displayName} files on GitHub`}
        >
          <svg
            aria-hidden="true"
            viewBox="0 0 24 24"
        ...
        {links.downloadUrl ? (
          <a
            ...
            aria-label={`Download ${port.displayName} release asset`}
          >
            <svg
              aria-hidden="true"
              viewBox="0 0 24 24"
