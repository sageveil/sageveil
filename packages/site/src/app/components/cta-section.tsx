...
export function CtaSection({ repoUrl }: CtaSectionProps) {
  return (
    <section className="grid items-center gap-7 md:grid-cols-[1fr_auto]">
      <div className="grid gap-4">
        <h2 className="text-[clamp(2rem,4vw,3.2rem)] font-light text-[var(--sv-bright-white)]">
          Open on GitHub.
        </h2>
        <p className="m-0 max-w-[48ch] text-[1.05rem] leading-[1.6] text-[var(--sv-muted)]">
          Browse the source, report issues, and track releases in the monorepo.
        </p>
      </div>
      <a
        className="inline-flex items-center justify-center gap-2 rounded-full bg-[var(--sv-accent)] px-7 py-3.5 text-[0.95rem] font-semibold text-[var(--sv-accent-ink)] transition-transform duration-200 hover:-translate-y-0.5 md:justify-self-end"
        href={repoUrl}
      >
        <svg
          aria-hidden="true"
